...
- 회사서 Amazon S3 용 VPC 게이트웨이 엔드포인트를 구성합니다**
    -> **VPC 에서 실행되는 애플리케이션 계층으로만 액세스를 제한하는 버킷 정책을 생성합니다.**

    -  Gateway Endpoint 는 퍼블릭 인터넷망을 통과하지 않는 전용 연결. 게이트웨이 엔드포인트는 VPC 용 인터넷 게이트웨이 또는 NAT 디바이스가 없는 애플리케이션의 탄력성과 가용성을 높이기 위해 애플리케이션을 AWS로 마이그레이션하고 있습니다

**AWS S3 VPC 게이트웨이 엔드포인트, 버킷 정책 생성**
